...
local theme_dir = string.format("%s/.config/awesome/themes/", os.getenv("HOME"))
local chosen_theme = "multicolor"
local theme_path = string.format(theme_dir .. "%s/theme.lua", chosen_theme)
...
-- {{{ Wibar
-- Create a volume widget with APW
...
awful.key({modkey}, "r", function() awful.util.spawn("rofi -show run") end,
          {description = "run rofi run", group = "launcher"}), -- RoFi
...
-- Screenshot
    ...
    awful.util.spawn("scrot '%Y-%m-%d,%H:%m:%S.png' -e 'mv $f ~/Screenshots/")
end, {description = "run rofi combi", group = "launcher"}),
                              awful.key({"Control", "Shift"}, "4", function()
    awful.util.spawn(
        "scrot '%Y-%m-%d,%H:%m:%S.png' -s -e 'mv $f ~/Screenshots/'")
end, {description = "run rofi combi", group = "launcher"}))
